...
def stream_detail(request, stream_id):
    ...
    chat_messages = ChatMessage.objects.filter(stream=stream).order_by('created_at')[:50]
    return render(request, 'stream_detail.html', {'stream': stream, 'chat_messages': chat_messages})
...
def send_chat_message(request, stream_id):
    if request.method == 'POST':
        ...
        ChatMessage.objects.create(stream=stream, sender=sender, message=message)
